const els = {
  ...
  meterLeft: document.querySelector("#meterLeft"),
  meterRight: document.querySelector("#meterRight"),
...
function renderSyncedLyrics(track, lyrics, lyricOffsetSeconds) {
  ...
  els.lyricsText.className = "v2-lyrics-text synced";
...
function renderPlainLyrics(lyrics, lyricScroll) {
  ...
  els.lyricsText.className = "v2-lyrics-text plain";
...
function renderLyrics(track, state) {
  ...
  if (!track) {
    ...
    els.lyricsText.className = "v2-lyrics-text";
...
function renderLevel(level) {
  ...
  if (Array.isArray(level?.spectrumBands) && level.spectrumBands.length) {
    spectrumState.bands = level.spectrumBands.map((value) => {
      const boosted = Math.pow(clamp(Number(value) || 0, 0, 1), 0.58) * 1.3;
...
function prepareCanvas(canvas) {
  ...
  return { ctx: canvas.getContext("2d"), scale, width, height };
}

function drawWaveform() {
  const { ctx, scale, width, height } = prepareCanvas(els.waveform);
  ...
  visiblePoints.forEach((value, index) => {
    ...
    const shaped = Math.pow(clamp(value, 0, 1), 1.72);
...
function drawSpectrum() {
  if (els.spectrumWrap?.classList.contains("hidden")) return;

  const { ctx, scale, width, height } = prepareCanvas(els.spectrum);
  ctx.clearRect(0, 0, width, height);
  ctx.fillStyle = "#161513";
  ctx.fillRect(0, 0, width, height);
  ...
  bands.forEach((target, index) => {
    ...
    const shaped = Math.pow(clamp(value, 0, 1), 0.82);
    const barHeight = Math.max(2 * scale, shaped * maxBarHeight);
    const x = index * (barWidth + gap);
